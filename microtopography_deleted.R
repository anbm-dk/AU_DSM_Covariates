# Elements removed from microtopography
...
#       }
#     }
#     // if center is NA, both counts stay 0 — same a na.rm=TRUE in R
...
mingab3 <- max(mingab, 0)
maxgab3 <- max(maxgab, 0)

sqrt(mingab3 * maxgab3) %>% plot()
...
plot(log(maxgab3 + 1) * log(mingab3 + 1))

plot(min(log(maxgab3 + 1), log(mingab3 + 1)))

plot(min(maxgab, mingab))

plot(sqrt(maxgab3 * mingab3)) # This one is good

ifel(
  min(maxgab, mingab) > 0,
  sqrt(min(maxgab, mingab)),
  -sqrt(-min(maxgab, mingab))
) %>%
  plot()

plot(sqrt(min(maxgab3, mingab3)))

plot(min(maxgab3, mingab3) / max(maxgab3, mingab3))

plot(min(maxgab3, mingab3) * sdgab / max(maxgab3, mingab3))

log(prod(abs(rast(outrasters)))) %>% plot() # Also very interesting
